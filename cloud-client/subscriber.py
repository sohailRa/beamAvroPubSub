...
# -------------------Start Custom Avro Receiver ------------------------------
def receive_avro_messages(project_id, subscription_name, timeout=None):
    """Receives messages from a pull subscription."""
    # [START pubsub_subscriber_async_pull]
    # [START pubsub_quickstart_subscriber]
    from google.cloud import pubsub_v1
    ...
    def callback(message):
        ...
        dict_record = schemaless_reader(bytes_reader, schema)
        print("Received avro to json converted message: {}".format(dict_record))
        message.ack()

    streaming_pull_future = subscriber.subscribe(
        subscription_path, callback=callback
    )
    print("Listening for avro messages on {}..\n".format(subscription_path))
...
if __name__ == "__main__":
    ...
    receive_avro_parser = subparsers.add_parser(
        "receive-avro", help="custom avro receiver"
    )
    receive_avro_parser.add_argument("subscription_name")
    receive_avro_parser.add_argument("--timeout", default=None, type=float)
    ...
    elif args.command == "receive-avro":
        receive_avro_messages(args.project_id, args.subscription_name, args.timeout)
